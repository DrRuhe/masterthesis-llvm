...
- [x] Write `test/smoke/static-global-basic.cpp`: target reads `static int x = 42`; verifies `assertSpecializedIsEquivalent` (SC-001/FR-006)
- [x] Write `test/smoke/static-global-modified.cpp`: increment static to 7 post-init; `specializeOrFallback`; FileCheck verifies result == 7 (SC-002)
- [x] Write `test/smoke/static-global-constructor.cpp`: constructor fills `static double data[8]` with `i * 1.5`; target reads `data[idx]`; `assertSpecializedIsEquivalent` (SC-001)
- [x] Write `test/smoke/static-global-readwrite.cpp`: target does `accum += v`; after call verify `accum` == `v` in host (SC-003)
- [x] Write `test/smoke/static-global-p2-fold.cpp`: `static const int table[4]`; target reads `table[0]`; P2 run; FileCheck verifies `ret i32 10` (SC-004)
- [x] [complex] Debug any issues in the debug-mode bitcode round-trip path — names survive round-trip; all 5 tests pass under default (debug) build
- [x] Verify SC-001–SC-005: `ninja check-smoke-runtime-specializer`; all 5 new tests pass, no regressions
