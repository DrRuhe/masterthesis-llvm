...
9. **Given** `--no-mem-limits` is not supplied (the default), **When** `record_benchmark.py` launches the benchmark, **Then** the binary is wrapped with `systemd-run --scope -p MemoryMax=<mem-max> -p MemoryHigh=<mem-high>` using the configured limits (defaults: `MemoryMax=20G`, `MemoryHigh=18G`); if `systemd-run` is not found on the system a warning is printed and the benchmark runs without limits. **Given** `--no-mem-limits` is supplied, **When** `record_benchmark.py` launches the binary, **Then** no `systemd-run` wrapper is added.
...
- What happens when `systemd-run` is not found and memory limits are not disabled? → A warning is printed to stderr and the benchmark launches without a memory limit wrapper; the run proceeds normally.
...
- **FR-047**: `record_benchmark.py` MUST wrap every benchmark invocation with `systemd-run --scope -p MemoryMax=<mem-max> -p MemoryHigh=<mem-high>` by default, limiting the benchmark process tree's memory usage. The hard cap (`--mem-max`, default `20G`) and the soft high-water threshold (`--mem-high`, default `18G`) MUST be configurable via CLI. Passing `--no-mem-limits` MUST disable the wrapper entirely. If `systemd-run` is not found on `PATH`, the script MUST print a warning to stderr and run the benchmark without the wrapper; this MUST NOT cause a hard error. Memory limits apply in all invocation modes (standard, `--benchmarking-best-practice`); when best-practice taskset is also active, the `systemd-run` wrapper is the outermost prefix.
