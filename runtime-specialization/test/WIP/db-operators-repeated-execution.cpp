...
int main(int argc, char* argv[]) {
    ...
    uintptr_t Addr = clangRuntimeSpecializer::specializeFunctionForRepeatedExec<Fn_execute_query>(&execute_query, query_plan);
    auto SpecializedFnPtr = reinterpret_cast<R(*)()>(Addr);


    for (int i = SpecializedFnPtr(); i != -1 ; i = SpecializedFnPtr())
    {
