...
#ifndef CLANG_RUNTIME_SPECIALIZER_JITSCCP_JITSCCPSOLVER_H
#define CLANG_RUNTIME_SPECIALIZER_JITSCCP_JITSCCPSOLVER_H
...
// Pull in llvm types we need in this header without polluting callers.
using llvm::Argument;
...
using llvm::Type;
using llvm::Value;
using llvm::ValueLatticeElement;
...
struct ArgInfo {
  ...
  friend llvm::hash_code hash_value(const ArgInfo &A) {
    return llvm::hash_combine(llvm::hash_value(A.Formal),
                              llvm::hash_value(A.Actual));
  }
};

class JitSCCPInstVisitor;

//===----------------------------------------------------------------------===//
//
/// JitSCCPSolver - This interface class is a general purpose solver for Sparse
/// Conditional Constant Propagation (SCCP).
///
class JitSCCPSolver {
  std::unique_ptr<JitSCCPInstVisitor> Visitor;

public:
  JitSCCPSolver(const DataLayout &DL,
  ...
  /// inform the JitSCCPSolver that it should track loads and stores to the
...
#endif // CLANG_RUNTIME_SPECIALIZER_JITSCCP_JITSCCPSOLVER_H
