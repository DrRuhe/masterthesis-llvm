...
// batch_delta: processes n_rows rows in a single call; all layout params
// (including n_rows) are specialization constants captured in the lambda.
static void batch_delta(const uint8_t* rows, int64_t n_rows,
...
BatchDeltaLowSpecialized create_batch_delta_low_specialized(
    ...
    auto lam = [n_rows, n_buckets, group_col_offset, value_col_offset, row_stride]
        ...
        batch_delta(rows, n_rows, n_buckets, group_col_offset, value_col_offset,
                    row_stride, sum_buckets);
    };
    return RS->specializeLambda<void>(lam);
}

void validate_batch_delta_low_specialized(int64_t n_rows, int n_buckets,
    ...
    batch_delta(test_data.data(), n_rows, n_buckets, group_col_offset,
                value_col_offset, row_stride, ref_buckets.data());

    // Specialized run.
    auto spec = create_batch_delta_low_specialized(
    ...
    for (int b = 0; b < n_buckets; ++b) {
        if (ref_buckets[b] != spec_buckets[b]) {
            throw std::runtime_error(
                "validate_batch_delta_low_specialized: bucket mismatch at index " +
                std::to_string(b));
        }
    }
}
