...
// Batch: loop inside function so benchmark harness calls once per iteration.
static void multi_agg_delta_batch_impl(const uint8_t* rows, int64_t n_rows,
                                        int n_buckets, int group_col_offset,
                                        int value_col_offset, int row_stride,
                                        double* sum_buckets, double* count_buckets) {
    for (int64_t i = 0; i < n_rows; ++i)
        multi_agg_delta(rows + static_cast<size_t>(i) * row_stride,
...
void multi_agg_delta_batch(const uint8_t* rows, int64_t n_rows,
                            ...
                            int n_buckets, int group_col_offset,
                            int value_col_offset, int row_stride) {
    multi_agg_delta_batch_impl(rows, n_rows, n_buckets, group_col_offset,
                               value_col_offset, row_stride, sum_buckets, count_buckets);
}
...
void validate_multi_agg_delta_batch_low_specialized(
    ...
    multi_agg_delta_batch_impl(test_data.data(), n_rows, n_buckets, group_col_offset,
                                value_col_offset, row_stride, ref_sum.data(), ref_cnt.data());
    ...
    for (int b = 0; b < n_buckets; ++b) {
        if (ref_sum[b] != spec_sum[b] || ref_cnt[b] != spec_cnt[b]) {
            throw std::runtime_error(
                "validate_multi_agg_delta_batch_low_specialized: bucket mismatch at index " +
                std::to_string(b));
        }
    }
}
