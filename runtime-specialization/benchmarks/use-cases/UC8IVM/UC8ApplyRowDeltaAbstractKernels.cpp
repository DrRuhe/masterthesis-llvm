...
ApplyRowDeltaBatchAbstractSpecialized create_apply_row_delta_batch_abstract_specialized(
    ...
    auto lam = [n_rows, group_col_offset, value_col_offset, n_buckets, row_stride](
                   const uint8_t* rows, double* state) {
        SumAggregator agg{group_col_offset, value_col_offset, n_buckets, row_stride};
        for (int64_t i = 0; i < n_rows; ++i)
            agg.apply(rows + static_cast<size_t>(i) * row_stride, state);
    };
    ...
    for (int b = 0; b < n_buckets; ++b) {
        if (ref_buckets[b] != spec_buckets[b]) {
            throw std::runtime_error(
                "validate_apply_row_delta_batch_abstract_specialized: bucket mismatch at index " +
                std::to_string(b));
        }
    }
}
...
void apply_row_delta_batch_abstract_unspecialized(const uint8_t* rows, int64_t n_rows,
    ...
    SumAggregator agg{group_col_offset, value_col_offset, n_buckets, row_stride};
    for (int64_t i = 0; i < n_rows; ++i)
        agg.apply(rows + static_cast<size_t>(i) * row_stride, buckets);
}
