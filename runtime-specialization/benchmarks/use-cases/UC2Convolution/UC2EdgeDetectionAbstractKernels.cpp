...
// Abstract gradient detector interface. Both base and concrete subclass live
...
struct SobelDetector : GradientDetector {
    ...
    void detect(const float* src, float* dst, int width, int height) override {
...
void validate_edge_detection_abstract_specialized() {
    ...
    SobelDetector ref_sd{};
    ref_sd.detect(src.data(), ref_dst.data(), W, H);

    auto spec = create_edge_detection_abstract_specialized(W, H);
    ...
    for (int i = 0; i < W * H; ++i) {
        ...
        if (diff < -kTol || diff > kTol)
            throw clangRuntimeSpecializer::ClangRuntimeSpecializerError(
                "validate_edge_detection_abstract_specialized: element-wise mismatch");
    }
    clangRuntimeSpecializer::ClangRuntimeSpecializer::log(
        clangRuntimeSpecializer::ClangRuntimeSpecializer::LogLevel::Info,
        "validate_edge_detection_abstract_specialized passed");
}
