...
// grouped_minmax: row_stride, key_offset, value_offset, n_buckets are specialization constants.
// Single-pass: extracts int32_t key and double value, updates min/max buckets.
// Caller must initialise min_buckets to DBL_MAX and max_buckets to -DBL_MAX.
void grouped_minmax(const uint8_t* rows, int64_t n_rows,
    ...
    for (int64_t i = 0; i < n_rows; ++i) {
        ...
        if (value < min_buckets[bucket]) min_buckets[bucket] = value;
        if (value > max_buckets[bucket]) max_buckets[bucket] = value;
    }
}
...
GroupedMinMaxLowSpecialized create_grouped_minmax_low_specialized(
    ...
    auto lam = [row_stride, key_offset, value_offset, n_buckets]
               (const uint8_t* rows, int64_t n_rows, double* min_out, double* max_out) {
        grouped_minmax(rows, n_rows, row_stride, key_offset, value_offset,
                       n_buckets, min_out, max_out);
    };
...
void validate_grouped_minmax_low_specialized(
    ...
    grouped_minmax(test_data.data(), N_TEST, row_stride, key_offset, value_offset,
                   n_buckets, ref_min.data(), ref_max.data());

    auto spec = create_grouped_minmax_low_specialized(row_stride, key_offset, value_offset, n_buckets);
    ...
    for (int b = 0; b < n_buckets; ++b) {
        if (ref_min[b] != spec_min[b] || ref_max[b] != spec_max[b]) {
            throw std::runtime_error(
                "validate_grouped_minmax_low_specialized: bucket mismatch at index " +
                std::to_string(b));
        }
    }
}
