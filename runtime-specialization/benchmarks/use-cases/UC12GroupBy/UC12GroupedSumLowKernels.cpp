#include "UC12Kernels.h"
#include "ClangRuntimeSpecializer.h"
#include <cstring>
#include <vector>
#include <stdexcept>

// CRITICAL: grouped_sum must live in the same TU as the lambda factory
// so the JIT module clone contains both for full inlining.
void grouped_sum(const uint8_t* rows, int64_t n_rows,
                 int row_stride, int key_offset, int value_offset,
                 double* out_buckets, int n_buckets) {
    for (int64_t i = 0; i < n_rows; ++i) {
        const uint8_t* row_ptr = rows + (int64_t)i * row_stride;

        int32_t key;
        std::memcpy(&key, row_ptr + key_offset, sizeof(int32_t));
        int bucket = ((key % n_buckets) + n_buckets) % n_buckets;

        double value;
        std::memcpy(&value, row_ptr + value_offset, sizeof(double));
        out_buckets[bucket] += value;
    }
}

GroupBySpecialized create_groupby_specialized(int row_stride, int key_offset,
                                               int value_offset, int n_buckets) {
    auto* RS = clangRuntimeSpecializer::ClangRuntimeSpecializer::init();
    auto lam = [row_stride, key_offset, value_offset, n_buckets]
               (const uint8_t* rows, int64_t n_rows, double* out_buckets) {
        grouped_sum(rows, n_rows, row_stride, key_offset, value_offset, out_buckets, n_buckets);
    };
    return RS->specializeLambda<void>(lam);
}

void validate_groupby_specialized(int row_stride, int key_offset,
                                   int value_offset, int n_buckets) {
    constexpr int N_TEST = 1000;
    std::vector<uint8_t> test_data(N_TEST * row_stride, 0);

    // Fill rows with known int32 keys and double values.
    // Key for row i = i % n_buckets (to spread across all buckets).
    // Value for row i = 1.0 (so each bucket accumulates its count as a double).
    for (int i = 0; i < N_TEST; ++i) {
        int32_t key = (int32_t)(i % n_buckets);
        std::memcpy(test_data.data() + i * row_stride + key_offset, &key, sizeof(int32_t));
        double value = 1.0;
        std::memcpy(test_data.data() + i * row_stride + value_offset, &value, sizeof(double));
    }

    std::vector<double> ref_buckets(n_buckets, 0.0);
    grouped_sum(test_data.data(), N_TEST, row_stride, key_offset, value_offset,
                ref_buckets.data(), n_buckets);

    auto spec = create_groupby_specialized(row_stride, key_offset, value_offset, n_buckets);

    std::vector<double> spec_buckets(n_buckets, 0.0);
    spec(test_data.data(), N_TEST, spec_buckets.data());

    for (int b = 0; b < n_buckets; ++b) {
        if (ref_buckets[b] != spec_buckets[b]) {
            throw std::runtime_error(
                "validate_groupby_specialized: bucket mismatch at index " +
                std::to_string(b));
        }
    }
}
