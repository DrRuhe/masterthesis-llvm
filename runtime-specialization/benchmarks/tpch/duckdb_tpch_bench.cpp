...
// duckdb_execute_prepared is defined in duckdb.cpp (the amalgamation TU compiled
// with the IR-dumping plugin). The extern "C" declaration here makes it visible
// for benchmarkJITAnalysis without pulling in a second definition.
...
// ── Global result slot ────────────────────────────────────────────────────────
// Address baked into the specialized function alongside the prepared_statement
// pointer. Destroyed and reused between loop iterations.
...
// ── DuckDB SQL query strings (standard TPC-H) ─────────────────────────────────
// DuckDB supports native SQL date arithmetic and EXTRACT.

// Q1: Pricing Summary Report
static constexpr const char* DUCKDB_Q1 = R"sql(
...
static constexpr const char* DUCKDB_Q2 = R"sql(
...
static constexpr const char* DUCKDB_Q3 = R"sql(
SELECT l_orderkey,
...
static constexpr const char* DUCKDB_Q4 = R"sql(
...
static constexpr const char* DUCKDB_Q5 = R"sql(
...
static constexpr const char* DUCKDB_Q6 = R"sql(
...
// Q7: Volume Shipping
static constexpr const char* DUCKDB_Q7 = R"sql(
...
static constexpr const char* DUCKDB_Q8 = R"sql(
SELECT o_year,
...
static constexpr const char* DUCKDB_Q9 = R"sql(
...
static constexpr const char* DUCKDB_Q10 = R"sql(
...
static constexpr const char* DUCKDB_Q11 = R"sql(
...
static constexpr const char* DUCKDB_Q12 = R"sql(
SELECT l_shipmode,
...
static constexpr const char* DUCKDB_Q13 = R"sql(
...
static constexpr const char* DUCKDB_Q14 = R"sql(
...
static constexpr const char* DUCKDB_Q15 = R"sql(
...
static constexpr const char* DUCKDB_Q16 = R"sql(
...
static constexpr const char* DUCKDB_Q17 = R"sql(
...
static constexpr const char* DUCKDB_Q18 = R"sql(
...
static constexpr const char* DUCKDB_Q19 = R"sql(
...
static constexpr const char* DUCKDB_Q20 = R"sql(
...
static constexpr const char* DUCKDB_Q21 = R"sql(
...
static constexpr const char* DUCKDB_Q22 = R"sql(
...
// Phase 1: Unspecialized — normal duckdb_execute_prepared() execution.
...
// g_last_jit_timed_out is set here so downstream phases can skip if JIT timed out.
...
// Phase 3: Specialized exec — compile once, run in loop, compare result to unspecialized.
static void phaseSpecializedExec(benchmark::State& state, const char* sql) {
    ...
    // Compare last specialized result to one fresh unspecialized execution.
