  ...
  for (auto& F : M) {
    ...
    for (auto& BB : F) {
      for (auto& I : BB) {
        ...
        // The user-facing overload takes (F* func, args...) where func is a
        // compile-time constant.  Find the function pointer argument.
        // If no Function* arg is found, this is the old string-based overload
        // (const char* funcName) — skip it silently; the pass only rewrites the
        // new funcptr overloads.
